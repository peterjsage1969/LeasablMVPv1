...
const SmartStockBatchDetailsEdit = (props) => {
    ...
    const setSubmitHandler = async (evt) => {
        ...
        // NEW
            if (nameEdit === '' || dateEdit === '' || associatedLeasableCategoryEdit === '' || initialSmartTechnology === '' || initialStockNumber === '') {
                setSuccess(false);
                ...
                try {
                    const response = await api.post(`/smartstockbatches`,smartStockBatchData, {headers: {user_id}})
