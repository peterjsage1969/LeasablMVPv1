...
import AccessoriesDelete from "./accessoriesDelete"
...
// PAGE DATA
    // collect
        useEffect(() => { 
            ...
            api.get(`/accessories/${productId}`,{headers: {user_id}})
            .then(res => {
    ...
    // populate
        useEffect(() => {  
            ...
            if(productData.category && productId != ''){setCategoryId(productData.category)} else {setCategoryId('')};
...
// ADD, EDIT & DELETE
    ...
    function submitHandler(props) {
        if (props.crudType === "add") {
            ...
            api.post(`/accessories`,productUpdate, {headers: {user_id}})
            .then(res => {
                setProductData(res.data)
    ...
    // edit
        if (props.crudType === "edit") {
            ...
            productUpdate.append("description", props.description)
            if (props.thumbnail){productUpdate.append("thumbnail", props.thumbnail) }
            api.post(`/accessories/update/${productId}`,productUpdate, {headers: {user_id}})
            .then(res => {
                setProductData(res.data)
                setUpdateStatus("complete");
                const interval = setInterval(() => {
    ...
    {popupStatus != ""?
        <div>
            <Modal isOpen={modal} toggle={toggle} centered={true} contentClassName="custom-modal-style">
                ...
                <ModalBody>
                    { popupStatus === "delete"?<AccessoriesDelete/>:null}            
                </ModalBody>
                </Modal>
        </div>
        : null}
